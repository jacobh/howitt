...
## GraphQL resolver observability

The API schema installs `ResolverTracing`, an async-graphql extension that creates custom Cloudflare spans named `graphql.resolve ParentType.field`. Span attributes are limited to `graphql.parent_type`, `graphql.field.name`, `graphql.return_type`, and `graphql.outcome` (`ok`, `error`, or `cancelled`). Arguments, aliases, operation names, query text, returned values and error messages are not recorded. Introspection and per-list-element serialization hooks are excluded, so GPS coordinates do not each create spans.

`src/lib/howitt-observability/src/span.js` bridges Wasm to the [Cloudflare custom-span API](https://developers.cloudflare.com/workers/observability/traces/custom-spans/). A request-owned async-context snapshot is restored for **every Rust future poll**; merely opening a span before an `.await` would attribute concurrent resolvers incorrectly. Spans close on success, error and cancellation, and inherit the Worker's trace sampling. The `nodejs_als` compatibility flag enables the small AsyncLocalStorage bridge without enabling full Node compatibility. Native builds emit equivalent Rust `tracing` spans.

Resolver durations are inclusive of nested resolution and I/O, not exclusive CPU time. Child resolver spans and automatic KV/fetch spans nest under the active resolver. Shared DataLoader work can belong to the resolver that initiates the batch rather than every waiter. Repository and SQL instrumentation is described below. Cloudflare's custom-span API does not yet support setting outcome status, so inspect `graphql.outcome` rather than relying on the span's platform success/error badge. Resolver-level tracing adds span volume; it uses the existing sampling configuration without changing it.

Validation: `cargo test -p howitt-web --lib --test resolver_observability` verifies native field/error instrumentation, parentage, unchanged output and metadata exclusions. `bun run test:observability` checks the JS bridge's context isolation across interleaved polls and unsampled behavior. `bash scripts/test-worker-local.sh` queries the real local observability store and verified 431 resolver spans with attributes/outcomes/parent links and 134 KV reads nested under resolvers. Wasm build/dry-run and clippy passed. API deployment `7f3ac4ee-49a3-4293-a243-0621a6a795b8` passed a live read-only Alpine Way query (HTTP 200, original 519 points).

## Repository and PostgreSQL observability

The shared `howitt-observability` crate supplies the same async-aware spans to all three layers:

- `graphql.execute` anchors the full execution future; `graphql.resolve Parent.field` spans retain resolver-level detail.
- `repo.Ride.filter_models`, `repo.User.put`, etc. wrap the six shared `AnyhowRepo` operations. They include connection waiting, queries, decoding, and nested repository logic. Attributes: `repo.model`, `repo.operation`, `repo.outcome`.
- `postgres.connection.wait` measures **mutex acquisition only**, independently of SQL execution. Native reconnects have a separate `postgres.connection.reconnect` span.
- `postgres.query_typed`, `postgres.query_typed_one`, `postgres.query_typed_opt`, and `postgres.execute_typed` measure each awaited statement. `postgres.transaction.begin`, `.commit`, and `.rollback` cover explicit transaction control. Automatic rollback on transaction drop is still handled by tokio-postgres and has no separate span.

The connection/transaction wrappers keep typed unnamed statements and explicit transaction locking; they do not expose a dereference bypass to the raw client. There is no query replay or new connection concurrency. SQL attributes are limited to `db.system.name = postgresql`, a bounded `db.operation.name` (SELECT/INSERT/UPDATE/DELETE/WITH/OTHER or connection/transaction control), and `db.outcome`. **No SQL text, parameters, connection strings, row values or error messages are emitted.** The span name and enclosing repository identify the operation; these are client-observed durations, not PostgreSQL server execution plans.

DataLoader and connection-driver tasks explicitly capture tracing context at spawn time and restore it on every poll. This matters because wasm-bindgen's shared task queue can otherwise run a batch in an unrelated repository's async context. In-flight contexts remain request-owned.

Validation includes bounded SQL-classification tests, native repository codecs/transactions/rollback/reconnect checks against a disposable database, and real Wasm trace-store assertions. The local harness verified 431 resolver spans, 88 repository spans and 178 SQL/connection-wait spans, including nonzero lock waiting, transaction BEGIN/COMMIT, metadata allowlists and repository/SQL parent links. Resolver instrumentation tests run in their own integration-test binary to isolate tracing subscriber/callsite state from concurrent unit tests. Deployed API version `ad48296c-ff20-4818-9bc5-1dac37767136`; the live frontend route-list browser smoke test passed.
...
Timezone lookup preserves `tzf-rs 0.4.11`'s preindex/polygon/coordinate-shift order. The preindex is loaded lazily only when a timezone field is requested; full polygons are loaded only for a preindex miss. Parsed immutable data is cached per isolate, not per request. A request-scoped async initialization lock coalesces concurrent cold lookups for both datasets, rechecking the completed cache after acquiring the lock. No in-flight I/O or request bindings are shared between requests. Asset/decode failures are errors, not guessed timezones; failures and cancellation leave initialization retryable.

The original cold loader allowed 64 concurrent resolvers to each fetch and parse the 2 MB preindex (trace `5f79e2d80b9624be1c7b0b379f7f82eb`: 10.4 seconds span duration, 8.0 seconds CPU). Fixed in API deployment `c7baac61-4d1d-4b1e-9071-ede4bb0daddd`. Regression tests verify one initialization per dataset for 64 overlapping lookups, completed-data reuse across request locks, and retry after failure/cancellation. The real local Wasm harness also exercises 64 timezone aliases per object, including polygon fallback, on consecutive HTTP requests. Live read-only queries with 64 trip timezone aliases returned consistent results on both requests; this smoke test does not reproduce the original full query or establish its post-fix CPU time.
