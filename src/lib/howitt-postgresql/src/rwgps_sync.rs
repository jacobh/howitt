use howitt::{
    ...
    services::sync::rwgps_v2::persistence::{RwgpsSyncResult, RwgpsSyncStore},
};
...
fn invalid_input(message: &'static str) -> Box<dyn std::error::Error + Send + Sync> {
    std::io::Error::new(std::io::ErrorKind::InvalidInput, message).into()
}

fn route_external_id(route: &Route) -> Result<i32, Box<dyn std::error::Error + Send + Sync>> {
    match route.external_ref.as_ref().map(|reference| &reference.id) {
        Some(ExternalId::Rwgps(RwgpsId::Route(id))) => Ok((*id).try_into()?),
        _ => Err(invalid_input(
            "RWGPS route persistence requires a route external reference",
        )),
    }
}

fn trip_external_id(ride: &Ride) -> Result<i32, Box<dyn std::error::Error + Send + Sync>> {
    match ride.external_ref.as_ref().map(|reference| &reference.id) {
        Some(ExternalId::Rwgps(RwgpsId::Trip(id))) => Ok((*id).try_into()?),
        _ => Err(invalid_input(
            "RWGPS trip persistence requires a trip external reference",
        )),
    }
}
        ...
        let id = if let Some(row) = existing {
            ...
            if user_id != *route.user_id.as_uuid() {
                return Err(invalid_input(
                    "RWGPS route external reference belongs to another user",
                ));
            }
            let stored: serde_json::Value = row.try_get("external_ref")?;
    ...
    async fn save_trip(&self, ride: Ride, points: Vec<TemporalElevationPoint>) -> RwgpsSyncResult {
        ...
        let id = if let Some(row) = existing {
            ...
            if user_id != *ride.user_id.as_uuid() {
                return Err(invalid_input(
                    "RWGPS trip external reference belongs to another user",
                ));
            }
            let stored: serde_json::Value = row.try_get("external_ref")?;
            let stored: howitt::models::external_ref::ExternalRef = serde_json::from_value(stored)?;
